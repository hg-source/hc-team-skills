...
name: agent-reach
description: >-
  인터넷 검색·조회와 지원 플랫폼의 공개 콘텐츠 수집을 담당한다. 사용자가 search,
  research, look up, 찾아줘, 검색해줘, 조사해줘 또는 특정 온라인 소스 확인을 요청할 때
  사용한다. 다중 소스 사업조사의 질문 설계·종합은 deep-search가 담당하고 agent-reach는
  자료 수집 계층으로 사용한다. 페이지 이미지 OCR·완전 아카이빙은 web-crawler-ocr,
  일반 접근 실패 후 우회는 web-bypass, YouTube 분석을 Notion에 저장하는 결과는
  youtube-to-notion을 우선한다. 이미 제공된 콘텐츠의 분석·번역이나 게시·댓글·좋아요 같은
  쓰기 작업에는 사용하지 않는다.
metadata:
